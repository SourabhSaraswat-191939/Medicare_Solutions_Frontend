      ...
      <section class="tableContainer">
        <div class="heading">
          ...
          <form method="GET" action="http://45.15.25.219/api/seller/download">
            <input type="text" value="4" name="index" hidden/>
            <button type="submit" class="download">Active Orders</button>
          </form>
        </div>
        <table role="table" class="table">
          <thead role="rowgroup">
            <tr role="row">
              ...
              <th role="columnheader">Out For Delivery</th>
          ...
          <tbody role="rowgroup">
            <tr role="row" v-for="(data,key) in serverData" :key="key">
              ...
              <td role="cell" data-title="Edit" style="color:#f76e2a;">
               <v-btn
                  elevated
                  color="success"
                  class="accept"
                   @click="active_order_action(data.id,key,'False')" 
...
export default {
  data(){
    ...
    methods:{
      active_order_action(id,key,delivery_out){
        API.deliver_Orders(id,delivery_out)
        .then(()=>{
          this.serverData.splice(key,1);
        })
      },
      getOrders(){
        API.getOrders(this.type)
        .then((response) => {
