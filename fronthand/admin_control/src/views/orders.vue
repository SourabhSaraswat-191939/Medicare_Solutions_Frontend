      ...
      <section class="tableContainer">
        <div class="heading">
          ...
          <form method="GET" action="http://45.15.25.219/api/seller/download">
            <input type="text" value="3" name="index" hidden/>
            <button type="submit" class="download">Orders</button>
          </form>
        </div>
        <table role="table" class="table">
          ...
          <tbody role="rowgroup">
            <tr role="row" v-for="(data,key) in serverData" :key="key">
              ...
              <td role="cell" data-title="Status">
              <v-btn
                  elevated
                  color="success"
                  class="accept"
                   @click="order_action(data.id,key,'True')" 
                ><v-icon dark>mdi-check-circle</v-icon>
                </v-btn><v-btn
                  elevated
                  color="error"
                  class="accept"
                  @click="order_action(data.id,key,'')"
...
export default {
  data(){
    ...
    methods:{
      download(){
        Axios.get(process.env.VUE_APP_SERVER+`/seller/download?index=1`)
        .then(()=>{
          console.log("ok")
        })
        .catch(()=>{
      ...
      order_action(id,key,acceptance_status){
        API.acceptOrder(id,acceptance_status)
        .then(()=>{
