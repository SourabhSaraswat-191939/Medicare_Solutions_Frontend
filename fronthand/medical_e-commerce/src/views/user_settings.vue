...
<center><div class="profile">
        ...
        <v-avatar color="indigo" size=108>
            <v-icon dark size=78>
                mdi-account-circle
            </v-icon>
        </v-avatar>
       ...
       <notifications position="center" group="input_error"/>
        ...
        <div class="details">
            ...
            <br>
            <h2>Pin Code</h2>
            <input type="text" id="p_code" class="input" v-model="pin_code" placeholder="Enter Pin Code" disabled>
            <button @click="toggle('p_code',edit,$event)" class="btn">Edit</button>
            <br><br>
            <h2>My Address</h2>
...
export default {
    ...
    mounted(){
        ...
        this.pin_code = this.$store.state.pin_code
    },
    methods:{
        ...
        update_info(){
            ...
            if((this.user_firstname).length && (this.user_lastname).length && (this.address).length && (this.mobile) && (this.pin_code))
            {
                API.update_info(this.user_firstname,this.user_lastname,this.mobile,this.address,this.$store.state.email,this.pin_code)
                .then(()=>{
